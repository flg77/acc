...
| `ACC_MLFLOW_TRACKING_URI` | _(env-only)_ | _(empty)_ | MLflow run/trace-logging endpoint; operator-injected into TUI/WebGUI pods |
| `ACC_OVERSIGHT_HEADLESS` | _(env-only)_ | `false` | Auto-reject CRITICAL actions when no reviewer is present (unattended / e2e) |
| `ACC_ROLE_TOKEN_BUDGET` | _(env-only)_ | _(empty)_ | Per-role Category-B token circuit-breaker (overrides the role's `token_budget`) |
