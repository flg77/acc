...
# ---------------------------------------------------------------------------
# CLI — the seam the shell assembler (acc-deploy.sh flavour) calls
# ---------------------------------------------------------------------------


def _resolve_from_args(args) -> ResolvedFeatures:
    if args.profile:
        return resolve_profile(args.profile)
    return resolve_features(str(args.features or "").split())


def _shellenv(r: ResolvedFeatures) -> str:
    """Emit `ACC_F_*` shell vars (space-joined) for `eval` in acc-deploy.sh."""
    def j(xs: list[str]) -> str:
        return " ".join(xs)
    return "\n".join(
        [
            f'ACC_F_FEATURES="{j(r.feature_ids)}"',
            f'ACC_F_EXTRAS="{",".join(r.extras)}"',  # comma for pip .[a,b]
            f'ACC_F_SKILLS="{j(r.skills)}"',
            f'ACC_F_MCPS="{j(r.mcps)}"',
            f'ACC_F_CHANNELS="{j(r.channels)}"',
            f'ACC_F_ACTIONS="{j(r.actions)}"',
            f'ACC_F_SIDECARS="{j(r.sidecars)}"',
            f'ACC_F_REQUIRES_ENV="{j(r.requires_env)}"',
            f'ACC_F_MODELS="{j(r.models)}"',
        ]
    )


def _env_required(r: ResolvedFeatures, features: dict[str, FeatureSpec]) -> str:
    """Emit a `.env.required` template: every required key + the optional ones
    (with their docs/defaults as comments). Keys-only — never a real value."""
    lines = [
        "# Generated by `acc-deploy.sh flavour` — fill these to activate the",
        "# baked features. Required keys are uncommented; optional ones carry",
        "# their default/description as a comment. NEVER commit real values.",
        "",
    ]
    if r.requires_env:
        lines.append("# --- required ---")
        lines += [f"{k}=" for k in r.requires_env]
        lines.append("")
    if r.optional_env:
        lines.append("# --- optional (defaults apply if unset) ---")
        for k in sorted(r.optional_env):
            lines.append(f"# {k}={r.optional_env[k]}")
    return "\n".join(lines) + "\n"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p = argparse.ArgumentParser(
        prog="acc.features",
        description="Resolve assistant build profiles / feature sets (proposal 043).",
    )
    sub = p.add_subparsers(dest="cmd", required=True)
    for name in ("shellenv", "env-required", "json"):
        sp = sub.add_parser(name)
        g = sp.add_mutually_exclusive_group(required=True)
        g.add_argument("--profile", help="profiles/<name>.yaml")
        g.add_argument("--features", help='space-separated feature ids, e.g. "messengers speech"')
    sub.add_parser("list")

    args = p.parse_args(argv)
    if args.cmd == "list":
        feats, profs = load_features(), load_profiles()
        print("features:", ", ".join(sorted(feats)))
        print("profiles:", ", ".join(f"{n}({s.kind})" for n, s in sorted(profs.items())))
        return 0

    try:
        r = _resolve_from_args(args)
    except ValueError as exc:
        print(f"error: {exc}", flush=True)
        return 2

    if args.cmd == "shellenv":
        print(_shellenv(r))
    elif args.cmd == "env-required":
        print(_env_required(r, load_features()), end="")
    elif args.cmd == "json":
        print(_json.dumps(r.__dict__, indent=2, sort_keys=True))
    return 0
