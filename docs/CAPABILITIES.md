...
| `acc-cli memory notes --hub` / `memory propose --to <scope|hub:<cid>>` / `memory curate --hub <cid> [--propose]` | The enterprise brain: a hub's **enterprise tier** fills only through approved publish proposals from the bound instances (quorum of two people, probation, ceiling carried; a HIGH / CRITICAL note needs **two distinct operator-tier approvals**, the rest one) and is the only cross-instance read path. `curate` lists the candidates; `propose` queues one for a person to approve. |
...
| `acc-cli compliance mappings/nexus/coverage/risks` | The frameworks and the threat model in the **Risk Atlas Nexus** vocabulary asago and OpenShift AI's EvalHub speak: an SSSOM export of every control → Nexus id mapping, the reverse lookup for a risk an asago policy extraction names, per-framework coverage against a pinned vocabulary, and `risks <risk-extraction.json>` — an asago policy dropped in: per extracted risk, the ACC controls and threats that answer to it and whether a loaded rule covers them (`docs/HOWTO-asago-policy.md`). |
| `acc` / `acc stack up\|down\|status` / `acc doctor` / `acc setup` / `acc paths` | The one command an operator starts with: the TUI attached to the running collective (exit 3 and a hint when nothing answers), the stack through the deploy script from the host's ACC layout, the doctor and the guided setup, and the resolved layout with the source of every path. |
| `acc tour` | The first-run tour: seven steps over the TUI — who you are here, the Prompt and the operating mode, the Board, Compliance with one sample gate to decide, your workspace, `/new-agent` — each stating the floor it keeps. Runs once on an installed layout, again on request, never in a checkout unasked. |
| `acc-prompt` decision panel (Prompt pane) | Defer (`d`) with a promise to come back before the deadline, hand off (`h`) to a person or tier, copy the id or command (`y`/`Y`), allow a LOW/MEDIUM class for 30 min, `ACC_PROMPT_LINEAR` reading order (2026-09-25). One request of one step renders as a full decision: the question, the numbered options, and beside them what the highlighted option actually does — consequence, scope, destination, risk, approvals still needed. `PENDING 1/2` where the decision is made. `n` notes onto the decision (kept on that approver's record), `c` asks about it while it stays pending, `Esc` later, `Ctrl+G` back. Several steps keep the compact request region. `ACC_PROMPT_PANEL=0` disables it. |
| `acc-cli workspace list/check/trust/deny/revoke` | The trusted-directory record `acc` asks about on start (`~/.config/acc/trust.yaml`: path, scope `dir`/`below`, decision, since, by). Trusting writes the sentinel the cells check; a super-repo (many repositories below) needs `--force`; a directory never trusted is never mounted, read or selected. |
| `acc-cli doctor --paths` | Where this host's ACC lives: home / share / state and every resolved path (`config`, `models`, `roles`, `skills`, `mcps`, `packages`, `instances`, …) with its source — `env`, `home`, `share`, `checkout`, `cwd` or the legacy `default`. |
...
| `acc-cli lessons list/show/trace <id>/send --to <agent> [--kind role_patch --set F=V]` | What agents told each other: every reflected note also rides `KNOWLEDGE_SHARE` as a typed `Lesson` (scope, ceiling, sources, dissent intact) and lands **once** in a peer's next prompt as `PEER_LESSONS`. `trace` joins a lesson to the tasks whose prompt rendered it and how they ended (a blocked task or a BAD / NEEDS_REVISE verdict lowers the lesson's confidence, GOOD raises it). A `role_patch` lesson is never applied: it becomes a `role_update` proposal in the oversight queue with the lesson as its evidence. A role also keeps a peer's note in its shared tier, after probation — on by default (D-028); `accept_peer_lessons: false` in a role's signed definition opts it out. `ACC_PEER_LESSONS=0` turns every direction off. |
| `acc-cli refine list/show/trace <id>/rollback <id> [--apply]` | The refinement ledger: one row for every durable change to what an agent knows or is — a reflected note, an adopted peer note, a published or hub-promoted note, a signed role update (which fields moved, from what to what), an approved learned rule, an erasure, an outcome — with its trigger, evidence, approver and ceiling. `trace` tells one note / lesson / role's whole story. `rollback` **is a dry run by default**: it revokes a note from every cache and the table, or queues a `role_update` proposal restoring the old values — a person approves the revert the way they approved the change. Erasures and rules are refused with the manual path named. |
| `acc-cli msg send <agent> --text … [--steer\|--follow-up\|--auto]` / `msg tail <agent>` / `msg show <id>` | Say something to one running agent on its inbox (`acc.{cid}.agent.{id}.inbox`; only the arbiter and the operator surface may publish there). `steer` lands in the prompt of the task in flight as `OPERATOR_STEERING`, right before the task and the last thing the context budget evicts; `follow_up` becomes a new task for that agent, run **as the sender** (tier and ceiling inherited). A steer that finds nothing in flight is delivered as a follow-up and the receipt says so. |
