...
the Prime Agent gap round (PA-03 P0, with PA-04, PA-02's inbox half and PA-09 Phase 1),
merged as #480. A lesson one agent learns now reaches its peers as a typed message, is
kept by default (D-028) under the ceiling and scope it was learned under, and every
durable change to what an agent knows or is lands in a ledger that can trace and roll
it back. Building it closed three governance gaps that were nobody's feature request:
nothing countersigned an unsigned `ROLE_UPDATE`, approved-proposal dispatch was
unauthorised in the NKey matrix for every kind, and a `can_route` role published an
arbiter-only subject. The last phase signs a worker's ask with the key it authenticates
its bus connection with, so the arbiter no longer believes a name inside the message.

OpenSpec `20260923-lessons-that-travel`, Phase 1 — a learning can now be handed
from one agent to another as a message. Raised by the Prime Agent gap round
(PA-03, its P0): `KNOWLEDGE_SHARE` was defined, permitted and documented, and
nothing published it.

### Added

- **`acc/lessons.py` — the `Lesson` envelope on `KNOWLEDGE_SHARE`.** Every memory note the reflection loop persists is also published on `acc.{cid}.knowledge.{tag}` with its RP-01 provenance intact (`scope`, `ceiling`, `source_requesters`, `evidence.source_episode_ids`, `dissent`). A peer with a matching `domain_receptors` entry validates it (a malformed envelope is dropped, never partially read) and renders it **once** into its next prompt as a `PEER_LESSONS` block after its own `MEMORY_NOTES`, under the same scope and ceiling rule the notes use and the same `memory_note_bandwidth`; hearsay is the first block the context budget evicts. Nothing a peer hears enters its hot cache or episode store.
- **`TASK_COMPLETE.lessons_used`** — the ids a turn's prompt rendered, indexed in Redis so a lesson can be joined to the tasks that saw it.
- **`acc-cli lessons list|show|trace <id>|send --to <agent>`** — the lessons published in the last 7 days, one in full, the tasks that rendered it, and an operator-addressed lesson to a single agent on the shared subject (the relay smoke test; no new bus subjects).
- Tracelog kind `lesson` in the agent's own `lessons-<agent_id>` journal (`published` / `received`).
- `ACC_PEER_LESSONS=0` turns publishing and rendering off; every prompt is then byte-identical to 0.21.0.

Phases 2–6 of the same change, on the same PR:

- **`acc/refinements.py` — the refinement ledger** (Phase 2). One append-only row (`refinements.jsonl` under the tracelog dir, mirrored 30 days in Redis) for every durable change to what an agent knows or is: a reflected note, an adopted peer note, a published / hub-promoted note, a signed `ROLE_UPDATE` (**which fields moved, from what to what**, on whose approval, citing the lesson that asked), an approved learned rule, an erasure, an outcome, a rollback. `acc-cli refine list|show|trace <id>|rollback <id> [--apply]` — `trace` tells one note's whole story; `rollback` (dry run by default) revokes a note from every cache and the table, or queues a `role_update` proposal restoring the old values through the oversight queue.
- **A `role_patch` lesson becomes a proposal, never an applied change** (Phase 3): the receiver queues one `role_update` proposal in the oversight queue with the lesson as its rationale; the dispatched `ROLE_UPDATE` carries `lesson_id` so the ledger row can cite it. `acc-cli lessons send --kind role_patch --set FIELD=VALUE`.
- **`accept_peer_lessons`** (Phase 4): a role field (signed `ROLE_UPDATE`, like `memory_note_bandwidth`) under which a note-kind lesson this role accepts also enters its shared tier for the lesson's scope — probation applies, so `acc-cli refine rollback` has its window. **On by default** (D-028): a role that says nothing keeps what its peers teach it; a role opts out with `accept_peer_lessons: false`. The adopted note keeps the lesson's scope and ceiling, and the read path filters on both. `ACC_PEER_LESSONS=0` still turns every direction off, and with it adoption.
- **Outcomes** (Phase 5): every task that rendered a lesson writes one outcome observation per lesson (`acc:{cid}:lesson:{id}:outcomes`, and a ledger row). Confidence moves on **objective** signals only — a blocked task or a BAD / NEEDS_REVISE verdict −0.10, GOOD +0.10; a task that merely finished is recorded and moves nothing. `lessons trace` shows them.
- **An authenticated sender** (Phase 10, PA-09 Phase 1). Phases 8 and 9 moved two worker-initiated control publishes behind the arbiter, and both then believed the `from_agent` / `from_role` fields in the payload — nothing tied either to the connection that carried the message. `acc/wire.py` has the sender sign a canonical form of the payload with **the key it authenticates its own NATS connection with**, and the receiver verify it against the key set `acc-nkeys generate` already writes; `acc/nkeys.py` gains the decoders for the format it has always encoded, so there is no new key material and no new dependency. Applied to `ROUTE_REQUEST`, checked before anything the payload claims about itself. Where the claimed role is itself an NKey identity the signer must be it; a packaged role presents a worker identity and cannot prove its role that way, so its signed role definition and the roster stay the bound — stated in the code rather than implied. A receiver with no key set readable accepts an unverified ask and logs it, so the rollout is sign → distribute → verify rather than a flag day. Inert unless `security.nkey.enabled`.
- **A `can_route` role asks, the arbiter routes** (Phase 9). An orchestrator re-dispatched by publishing a directed `TASK_ASSIGN` itself, and `acc.*.task.assign` is arbiter-only — so on an NKey-enforced deployment every orchestrator hand-off was refused by the server. The routing decision stays the orchestrator's; only the privileged publish moves. `ROUTE_REQUEST` on `acc.{cid}.route.request` is worker-publishable and arbiter-subscribed; the arbiter re-checks the requesting role's **signed** `can_route`, that the roster agrees it holds that role, and that the task is routable and not already routed. `build_routed_task()` is now the single builder both sides use. Inert unless `security.nkey.enabled`. Known bound, stated in the proposal: the sender is self-declared, so a member could claim a role it does not hold — the request can still only re-dispatch a task already in flight, once.
- **Who may dispatch an approved proposal** (Phase 8). Dispatching a proposal is a control-plane publish, and the identity that claims the approval is not always the one the NKey matrix lets make it — so the publish went out and the server refused it, silently. Measured against the matrix, every kind was affected: `role_update` / `route` refused for workers, and `collective.reconcile` (spawn) plus `assistant.proposal` (infuse / role_gap / publish) granted to **no identity at all**. Fixed three ways: `acc.*.assistant.*` joins the worker baseline (an announcement carries no mutation — without it an NKey deployment could not announce a pending proposal) and, with `acc.*.collective.reconcile`, the arbiter, so the arbiter may dispatch every kind; `dispatch_subjects(kind)` + `nats_permissions.may_publish()` make the question askable (all of a kind's subjects count — `infuse` also publishes the continuation `TASK_ASSIGN`); and the guard runs **before** the exactly-once claim, so an agent that may not publish leaves the approval for the arbiter instead of consuming it, while an AUTO execution it may not publish is queued for a human rather than dropped. Inert unless `security.nkey.enabled` — standalone behaves exactly as before. `tests/test_proposal_dispatch_authority.py` pins per-kind publish authority, including why `subject_covered()` never caught this (it passes on publish **or subscribe**, and the arbiter subscribes `acc.>`).
- **The arbiter countersigns** (Phase 7): an unsigned `ROLE_UPDATE` — an approved proposal's `{role, fields}` or `acc-cli role infuse`'s definition with an empty signature — used to reach every agent and end in each RoleStore's `rejected: missing_signature` row (pre-existing; found wiring Phase 3). The arbiter now resolves the role, applies the fields, bumps the version, signs the canonical message with `security.arbiter_signing_key` and re-publishes it with `countersigned_for` naming who decided; a signed update that names a role is applied by that role's agents only.
- **The critic join** (Phase 5, the arbiter's half): a reviewer step's verdict reaches the lessons the steps it `depends_on` used, recorded as `source: critic` — the one outcome an agent cannot give itself.
- **The harness fingerprint** (PA-04): sha256 over role version, system prompt, notes, lessons and steering rendered, on every episode's `payload_json`, on `TASK_COMPLETE` and on every outcome observation, so a pass-rate that moves between fingerprints can be attributed to a refinement.
- **The inbox** (Phase 6): `acc.{cid}.agent.{agent_id}.inbox` (`AGENT_MESSAGE`, `acc/agent_messages.py`), publishable by the arbiter and the operator surface only (NKey matrix; workers subscribe to their own). `steer` lands in the task in flight as `OPERATOR_STEERING`, right before the task and evicted last; `follow_up` becomes a task for that agent run **as the sender** (attribution and ceiling inherited), through the agent's own task loop. Receipts in Redis and the `messages-<agent_id>` journal; `TASK_COMPLETE.steer_used`. `acc-cli msg send <agent> --text … [--steer|--follow-up|--auto]`, `msg tail`, `msg show`.

### Packaging

- **A standalone GPG signing key for the `ic3net_internal/ACC/acc-spearhead` channel** (#478): generated on lighthouse and wired into the Satellite as the content credential `acc-spearhead-signing`; `sign-rpms-standalone.sh` and `release-pipeline.sh --standalone-key` sign with it, and `publish-satellite.sh` prints `gpgcheck=1` once a repo carries a key. Signing itself still waits on `rpm-sign` being installed on lighthouse.
